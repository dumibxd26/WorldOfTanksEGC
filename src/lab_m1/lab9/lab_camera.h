...
namespace implemented
{
	...
	public:
		...
		void TranslateRight(float distance)
		{
			...
			// is not very useful, becauste if the camera is rotated around the
		...
		void RotateFirstPerson_OY(float angle)
		{
			...
			// vectors!
			glm::vec4 aux = glm::rotate(glm::mat4(1), angle, glm::vec3(0, 1, 0)) * glm::vec4(forward, 0);
			forward = glm::normalize(glm::vec3(aux));
		...
		void RotateFirstPerson_OZ(float angle)
		{
			...
			glm::vec4 aux = glm::rotate(glm::mat4(1), angle, forward) * glm::vec4(right, 0);
			right = glm::normalize(glm::vec3(aux));

			aux = glm::rotate(glm::mat4(1), angle, forward) * glm::vec4(up, 0);
			up = glm::normalize(glm::vec3(aux));
